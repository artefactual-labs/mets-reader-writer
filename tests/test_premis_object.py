...
class TestPremisObject(TestCase):
    ...
    def test_fromfile(self):
        ...
        premis_object = premis.Object.parse(object_el)
    ...
    def test_object_construction(self):
        with pytest.raises(metsrw.ConstructError):
            premis.Object(None, 'Category', None, None, [])
        with pytest.raises(metsrw.ConstructError):
            premis.Object([], None, None, None, [])
        with pytest.raises(metsrw.ConstructError):
            premis.Object([], 'Category', None, None, None)
    ...
    def test_parse_wrong_element(self):
        ...
        with pytest.raises(metsrw.ParseError):
            premis.Object.parse(event_el)

    def test_serialization(self):
        parser = etree.XMLParser(remove_blank_text=True)
        root = etree.parse('fixtures/complete_mets.xml', parser=parser)

        fixture_object_el = root.find("mets:amdSec[@ID='amdSec_17']/" +
                                      "mets:techMD[@ID='techMD_17']/" +
                                      "mets:mdWrap/" +
                                      "mets:xmlData/" +
                                      "premis:object", namespaces=utils.NAMESPACES)

        parsed_object = premis.Object.parse(fixture_object_el)
        parsed_object_el = parsed_object.serialize()

        fixture_object_xml = etree.tostring(fixture_object_el, pretty_print=True)
        parsed_object_xml = etree.tostring(parsed_object_el, pretty_print=True)

        assert fixture_object_xml == parsed_object_xml
