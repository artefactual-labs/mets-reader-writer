"""PREMIS-Reader-Writer 2 Event

Supplies a PREMIS ``Event`` class for reading and writing PREMIS events.
Supports the API required for a metsrw plugin, i.e., provides the following
methods:

1. A ``fromtree`` class method that parses an lxml element and returns a Python
   instance.

2. A ``serialize`` method that returns an lxml element.

The ``metsrw.FSEntry`` class can accept ``yapremisrw.Event`` as a dependency. To
inject it::

    >>> from metsrw import feature_broker
    >>> from metsrw.plugins import yapremisrw
...
class Event(object):
    ...
    :param list linking_agent_identifiers: List of dicts containing linking
        agent identifier data.
    :param list linking_object_identifiers: List of dicts containing linking
        object identifier data.
    :raises exceptions.ParseError: If the root tag is not event.
    """
    def __init__(self, identifier_type, identifier_value, event_type,
    ...
    def parse(cls, root):
        ...
        :raises ParseError: If the root is not event.
        """
        if root.tag != utils.lxmlns('premis') + 'event':
            raise ParseError('Event can only parse event elements with PREMIS namespace.')
    ...
    # So that ``premisrw.Event`` will conform to the API required by
    # ``metsrw.FSEntry``
    fromtree = parse
    ...
    def _parse_outcomes(cls, outcome_information_els):
        ...
        for outcome_information_el in outcome_information_els:
            ...
            detail_els = outcome_information_el.findall(
                "premis:eventOutcomeDetail", namespaces=utils.NAMESPACES)
