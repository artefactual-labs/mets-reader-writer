"""PREMIS-Reader-Writer 2 Object

Supplies a PREMIS ``Object`` class for reading and writing PREMIS objects.
...
The ``metsrw.FSEntry`` class can accept ``yapremisrw.Object`` as a dependency. To
inject it::
...
class Object(object):
    ...
    def __init__(self, object_identifiers, object_category, preservation_levels=None, significant_properties=None, characteristics=None, original_name=None, storage=None, environments=None, signature_information=None, relationships=None, linking_event_identifiers=None, linking_intellectual_entity_identifiers=None, linking_rights_statement_identifiers=None, strict=True):
        if strict and object_identifiers is None:
            raise ConstructError('object_identifiers argument is required.')

        if strict and object_category is None:
            raise ConstructError('object_category argument is required.')

        if strict and characteristics is None:
            raise ConstructError('characteristics argument is required.')
    ...
    def parse(cls, root, strict=True):
        ...
        if root.tag != utils.lxmlns('premis') + 'object':
            raise ParseError('Object can only parse object elements with PREMIS namespace.')
    ...
    @classmethod
    def fromtree(cls, root):
        return cls.parse(root, strict=False)
...
class ObjectCharacteristics(object):
    ...
    def parse(cls, root):
        ...
        if root.tag != utils.lxmlns('premis') + 'objectCharacteristics':
            raise ParseError('ObjectCharacteristics can only parse objectCharacteristics elements with PREMIS namespace.')
